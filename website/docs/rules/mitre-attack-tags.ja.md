...
タクティクスの略記はハードコードされておらず、実行時に `config/mitre_tactics.txt`（[Hayabusa](https://github.com/Yamato-Security/hayabusa) と共通のテーブル）から読み込まれます。各行は `<フルタグ>,<略記>` という単純な組であり、Suzaku を再ビルドせずに略記を編集・追加できます:
