# Perintah Ringkasan DFIR

## Perintah `aws-ct-summary`

Perintah ini membuat ringkasan dari informasi berikut berdasarkan ARN pengguna:
  - Jumlah total kejadian (panggilan API) (`NumOfEvents`)
  - Stempel waktu dari panggilan API pertama yang ditemukan dalam log (`FirstTimestamp`)
  - Stempel waktu dari panggilan API terakhir yang ditemukan dalam log (`LastTimestamp`)
  - Panggilan API yang umumnya disalahgunakan yang berhasil (`AbusedAPIs-Success`)
  - Panggilan API yang umumnya disalahgunakan yang dicoba tetapi gagal (`AbusedAPIs-Failed`)
  - Panggilan API lain yang tidak ada dalam daftar panggilan API yang umumnya disalahgunakan yang berhasil (`OtherAPIs-Success`)
  - Panggilan API lain yang tidak ada dalam daftar panggilan API yang umumnya disalahgunakan yang dicoba tetapi gagal (`OtherAPIs-Failed`)
  - Wilayah AWS tempat panggilan API dilakukan (`AWS-Regions`)
  - Alamat IP sumber dari panggilan API (`SrcIPs`)
  - Tipe pengguna (`UserTypes`)
  - ID kunci akses pengguna (`UserAccessKeyIDs`)
  - User agent dari sumber yang melakukan panggilan API (`UserAgents`)

※ Catatan: panggilan API yang umumnya disalahgunakan berasal dari file konfigurasi yang dihosting di [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv). File ini akan diperbarui dari waktu ke waktu dan akan disinkronkan secara lokal setiap kali Anda menjalankan perintah `update-rules`.

Hasil ini dimaksudkan untuk memberikan analis informasi guna menemukan akun yang dikompromikan atau serangan yang tidak bergantung pada signature tertentu.
Sebagai contoh, Anda dapat memeriksa apakah pengguna tertentu memanggil panggilan API mencurigakan yang seharusnya tidak mereka panggil, menggunakan wilayah yang biasanya tidak digunakan, masuk dari alamat IP sumber yang mencurigakan atau dengan user agent yang mencurigakan, dll...
Setelah Anda menemukan panggilan API yang mencurigakan dipanggil, alamat IP sumber atau user agent, Anda dapat dengan cepat menentukan kunci akses mana yang mungkin telah disalahgunakan selama rentang waktu tersebut dan melakukan pivot pada kata kunci tersebut dalam log JSON asli untuk membuat lini masa aktivitas penyerang.

> Peringatan: akan ada banyak data di dalam sel dan kemungkinan besar tidak akan ditampilkan dengan baik di program seperti Excel. Silakan gunakan Numbers di Mac, Timeline Explorer di Windows, dll...

### Contoh `AbusedAPIs-Success`:
```
...
> Perhatikan bahwa karena akan ada sejumlah besar ID kunci akses AWS STS sementara, ini disaring secara default. Tambahkan opsi `-s, --include-sts-keys` jika Anda ingin menyertakannya.
...
> Perhatikan bahwa alat klien `aws` akan menyertakan informasi OS dalam user agent sehingga dimungkinkan untuk mendeteksi apakah ada panggilan API yang dilakukan dari OS penyerang seperti `kali`.
...
* Menyimpan hasil ke file CSV: `./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
