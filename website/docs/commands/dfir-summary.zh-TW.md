...
此命令會根據使用者 ARN 建立以下資訊的摘要：
  - 事件（API 呼叫）總數（`NumOfEvents`）
  - 在日誌中找到的第一個 API 呼叫的時間戳記（`FirstTimestamp`）
  - 在日誌中找到的最後一個 API 呼叫的時間戳記（`LastTimestamp`）
  - 成功的常見遭濫用 API 呼叫（`AbusedAPIs-Success`）
  - 嘗試但失敗的常見遭濫用 API 呼叫（`AbusedAPIs-Failed`）
  - 不在常見遭濫用 API 呼叫清單上且成功的其他 API 呼叫（`OtherAPIs-Success`）
  - 不在常見遭濫用 API 呼叫清單上且嘗試但失敗的其他 API 呼叫（`OtherAPIs-Failed`）
  - 進行 API 呼叫所在的 AWS 區域（`AWS-Regions`）
  - API 呼叫的來源 IP 位址（`SrcIPs`）
  - 使用者類型（`UserTypes`）
  - 使用者存取金鑰 ID（`UserAccessKeyIDs`）
  - 進行 API 呼叫的來源使用者代理（`UserAgents`）

※ 注意：常見遭濫用的 API 呼叫來自託管於 [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv) 的設定檔。此檔案會隨時間更新，並會在您每次執行 `update-rules` 命令時於本機同步。

這些結果旨在為分析人員提供資訊，以發現遭入侵的帳戶或不依賴特定特徵碼的攻擊。
例如，您可以檢查某些使用者是否正在呼叫他們不應呼叫的可疑 API、使用通常不會使用的區域、從可疑的來源 IP 位址或以可疑的使用者代理登入等等……
在您發現任何被呼叫的可疑 API、來源 IP 位址或使用者代理後，您可以快速判斷在該時間範圍內哪些存取金鑰可能遭到濫用，並在原始 JSON 日誌中以這些關鍵字進行樞紐分析，以建立攻擊者活動的時間軸。

> 警告：儲存格中會有大量資料，且很可能無法在 Excel 等程式中正常顯示。請在 Mac 上使用 Numbers、在 Windows 上使用 Timeline Explorer 等等……

### `AbusedAPIs-Success` 範例：
```
...
> 請注意，由於會有大量暫時的 AWS STS 存取金鑰 ID，這些預設會被過濾掉。如果您想要包含這些金鑰，請加上 `-s, --include-sts-keys` 選項。
...
> 請注意，`aws` 用戶端工具會在使用者代理中包含作業系統資訊，因此可以偵測是否有從 `kali` 等攻擊者作業系統發出的 API 呼叫。
...
* 將結果儲存至 CSV 檔案：`./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
