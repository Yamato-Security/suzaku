# Comandos de Resumo DFIR

## Comando `aws-ct-summary`

Este comando cria um resumo das seguintes informações com base nos ARNs dos usuários:
  - Número total de eventos (chamadas de API) (`NumOfEvents`)
  - Timestamp da primeira chamada de API encontrada nos logs (`FirstTimestamp`)
  - Timestamp da última chamada de API encontrada nos logs (`LastTimestamp`)
  - Chamadas de API comumente abusadas que tiveram sucesso (`AbusedAPIs-Success`)
  - Chamadas de API comumente abusadas que foram tentadas mas falharam (`AbusedAPIs-Failed`)
  - Outras chamadas de API que não estão na lista de chamadas de API comumente abusadas que tiveram sucesso (`OtherAPIs-Success`)
  - Outras chamadas de API que não estão na lista de chamadas de API comumente abusadas que foram tentadas mas falharam (`OtherAPIs-Failed`)
  - Regiões da AWS onde as chamadas de API foram feitas (`AWS-Regions`)
  - Endereços IP de origem da chamada de API (`SrcIPs`)
  - Tipos de usuário (`UserTypes`)
  - IDs de chave de acesso de usuário (`UserAccessKeyIDs`)
  - User agents da origem que fez a chamada de API (`UserAgents`)

※ Observação: as chamadas de API que são comumente abusadas vêm do arquivo de configuração hospedado em [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv). Este arquivo será atualizado ao longo do tempo e será sincronizado localmente toda vez que você executar o comando `update-rules`.

Estes resultados têm como objetivo fornecer aos analistas informações para descobrir contas comprometidas ou ataques que não dependem de assinaturas específicas.
Por exemplo, você pode verificar se determinados usuários estão fazendo chamadas de API suspeitas que não deveriam estar fazendo, usando regiões que normalmente não são utilizadas, fazendo login a partir de endereços IP de origem suspeitos ou com user agents suspeitos, etc...
Depois de encontrar quaisquer chamadas de API suspeitas sendo feitas, endereços IP de origem ou user agents, você pode determinar rapidamente quais chaves de acesso podem ter sido abusadas durante aquele período e pivotar nessas palavras-chave nos logs JSON originais para criar uma linha do tempo da atividade do atacante.

> Aviso: haverá muitos dados nas células e provavelmente não serão exibidos adequadamente em programas como o Excel. Use o Numbers em um Mac, o Timeline Explorer no Windows, etc...

### Exemplo de `AbusedAPIs-Success`:
```
...
> Observe que, como haverá uma grande quantidade de IDs temporários de chave de acesso do AWS STS, estes são filtrados por padrão. Adicione a opção `-s, --include-sts-keys` se quiser incluí-los.

### Exemplo de `UserAgents`:
```
...
> Observe que a ferramenta cliente `aws` incluirá as informações do SO no user agent, então é possível detectar se houve chamadas de API feitas a partir de SOs de atacantes como o `kali`.
...
### Exemplo do comando `aws-ct-metrics`

* Salvar resultados em um arquivo CSV: `./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
