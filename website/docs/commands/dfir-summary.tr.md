...
## `aws-ct-summary` komutu

Bu komut, kullanıcı ARN'lerine dayanarak aşağıdaki bilgilerin bir özetini oluşturur:
  - Toplam olay (API çağrısı) sayısı (`NumOfEvents`)
  - Günlüklerde bulunan ilk API çağrısının zaman damgası (`FirstTimestamp`)
  - Günlüklerde bulunan son API çağrısının zaman damgası (`LastTimestamp`)
  - Başarılı olan, yaygın olarak kötüye kullanılan API çağrıları (`AbusedAPIs-Success`)
  - Denenip başarısız olan, yaygın olarak kötüye kullanılan API çağrıları (`AbusedAPIs-Failed`)
  - Yaygın olarak kötüye kullanılan API çağrıları listesinde olmayan ve başarılı olan diğer API çağrıları (`OtherAPIs-Success`)
  - Yaygın olarak kötüye kullanılan API çağrıları listesinde olmayan, denenip başarısız olan diğer API çağrıları (`OtherAPIs-Failed`)
  - API çağrılarının yapıldığı AWS bölgeleri (`AWS-Regions`)
  - API çağrısının kaynak IP adresleri (`SrcIPs`)
  - Kullanıcı türleri (`UserTypes`)
  - Kullanıcı erişim anahtarı kimlikleri (`UserAccessKeyIDs`)
  - API çağrısını yapan kaynağın kullanıcı aracıları (`UserAgents`)

※ Not: yaygın olarak kötüye kullanılan API çağrıları, [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv) adresinde barındırılan yapılandırma dosyasından gelir. Bu dosya zamanla güncellenecek ve her `update-rules` komutunu çalıştırdığınızda yerel olarak senkronize edilecektir.

Bu sonuçlar, analistlere belirli imzalara dayanmayan, ele geçirilmiş hesapları veya saldırıları keşfetmeleri için bilgi sağlamayı amaçlamaktadır.
Örneğin, belirli kullanıcıların çağırmamaları gereken şüpheli API çağrıları yapıp yapmadığını, tipik olarak kullanılmayan bölgeleri kullanıp kullanmadıklarını, şüpheli kaynak IP adreslerinden veya şüpheli kullanıcı aracılarıyla oturum açıp açmadıklarını vb. kontrol edebilirsiniz...
Çağrılan şüpheli API çağrılarını, kaynak IP adreslerini veya kullanıcı aracılarını bulduktan sonra, o zaman diliminde hangi erişim anahtarlarının kötüye kullanılmış olabileceğini hızlıca belirleyebilir ve saldırgan faaliyetinin bir zaman çizelgesini oluşturmak için orijinal JSON günlüklerinde bu anahtar kelimeler üzerinden inceleme yapabilirsiniz.

> Uyarı: hücrelerde çok fazla veri olacak ve büyük olasılıkla Excel gibi programlarda iyi görüntülenmeyecektir. Lütfen Mac'te Numbers, Windows'ta Timeline Explorer vb. kullanın...

### `AbusedAPIs-Success` örneği:
```
...
> AWS STS geçici erişim anahtarı kimliklerinden çok sayıda olacağından, bunlar varsayılan olarak filtrelenir. Bunları dahil etmek istiyorsanız `-s, --include-sts-keys` seçeneğini ekleyin.
...
> `aws` istemci aracının kullanıcı aracısında işletim sistemi bilgisini de dahil edeceğini unutmayın; bu nedenle `kali` gibi saldırgan işletim sistemlerinden API çağrıları yapılıp yapılmadığını tespit etmek mümkündür.
...
### `aws-ct-metrics` komut örneği

* Sonuçları bir CSV dosyasına kaydet: `./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
