# Comandos de resumen DFIR

## Comando `aws-ct-summary`

Este comando crea un resumen de la siguiente información basada en los ARN de usuario:
  - Número total de eventos (llamadas a la API) (`NumOfEvents`)
  - Marca de tiempo de la primera llamada a la API encontrada en los registros (`FirstTimestamp`)
  - Marca de tiempo de la última llamada a la API encontrada en los registros (`LastTimestamp`)
  - Llamadas a la API comúnmente abusadas que tuvieron éxito (`AbusedAPIs-Success`)
  - Llamadas a la API comúnmente abusadas que se intentaron pero fallaron (`AbusedAPIs-Failed`)
  - Otras llamadas a la API que no están en la lista de llamadas a la API comúnmente abusadas y que tuvieron éxito (`OtherAPIs-Success`)
  - Otras llamadas a la API que no están en la lista de llamadas a la API comúnmente abusadas y que se intentaron pero fallaron (`OtherAPIs-Failed`)
  - Regiones de AWS donde se realizaron las llamadas a la API (`AWS-Regions`)
  - Direcciones IP de origen de la llamada a la API (`SrcIPs`)
  - Tipos de usuario (`UserTypes`)
  - ID de claves de acceso de usuario (`UserAccessKeyIDs`)
  - Agentes de usuario del origen que realizó la llamada a la API (`UserAgents`)

※ Nota: las llamadas a la API que son comúnmente abusadas provienen del archivo de configuración alojado en [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv). Este archivo se actualizará con el tiempo y se sincronizará localmente cada vez que ejecute el comando `update-rules`.

Estos resultados están destinados a proporcionar a los analistas información para descubrir cuentas comprometidas o ataques que no dependen de firmas específicas.
Por ejemplo, puede comprobar si ciertos usuarios están realizando llamadas a la API sospechosas que no deberían realizar, utilizando regiones que normalmente no se usan, iniciando sesión desde direcciones IP de origen sospechosas o con agentes de usuario sospechosos, etc...
Después de encontrar cualquier llamada a la API sospechosa que se esté realizando, direcciones IP de origen o agentes de usuario, puede determinar rápidamente qué claves de acceso podrían haber sido abusadas durante ese período de tiempo y pivotar sobre esas palabras clave en los registros JSON originales para crear una cronología de la actividad del atacante.

> Advertencia: habrá muchos datos en las celdas y lo más probable es que no se muestren bien en programas como Excel. Utilice Numbers en un Mac, Timeline Explorer en Windows, etc...

### Ejemplo de `AbusedAPIs-Success`:
```
...
> Tenga en cuenta que, dado que habrá una gran cantidad de ID de claves de acceso temporales de AWS STS, estas se filtran de forma predeterminada. Agregue la opción `-s, --include-sts-keys` si desea incluirlas.

### Ejemplo de `UserAgents`:
```
...
> Tenga en cuenta que la herramienta cliente `aws` incluirá la información del sistema operativo en el agente de usuario, por lo que es posible detectar si hubo llamadas a la API realizadas desde sistemas operativos de atacantes como `kali`.
...
### Ejemplo del comando `aws-ct-metrics`

* Guardar los resultados en un archivo CSV: `./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
