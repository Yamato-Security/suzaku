# Команди DFIR Summary

## Команда `aws-ct-summary`

Ця команда створює зведення наступної інформації на основі ARN користувачів:
  - Загальна кількість подій (викликів API) (`NumOfEvents`)
  - Мітка часу першого виклику API, знайденого в журналах (`FirstTimestamp`)
  - Мітка часу останнього виклику API, знайденого в журналах (`LastTimestamp`)
  - Виклики API, якими зазвичай зловживають, що були успішними (`AbusedAPIs-Success`)
  - Виклики API, якими зазвичай зловживають, що були спробувані, але зазнали невдачі (`AbusedAPIs-Failed`)
  - Інші виклики API, яких немає в списку викликів API, якими зазвичай зловживають, що були успішними (`OtherAPIs-Success`)
  - Інші виклики API, яких немає в списку викликів API, якими зазвичай зловживають, що були спробувані, але зазнали невдачі (`OtherAPIs-Failed`)
  - Регіони AWS, де були здійснені виклики API (`AWS-Regions`)
  - IP-адреси джерела виклику API (`SrcIPs`)
  - Типи користувачів (`UserTypes`)
  - Ідентифікатори ключів доступу користувачів (`UserAccessKeyIDs`)
  - User agents джерела, яке здійснило виклик API (`UserAgents`)

※ Примітка: виклики API, якими зазвичай зловживають, походять із файлу конфігурації, розміщеного за адресою [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv). Цей файл оновлюватиметься з часом і синхронізуватиметься локально щоразу, коли ви запускаєте команду `update-rules`.

Ці результати призначені для надання аналітикам інформації для виявлення скомпрометованих облікових записів або атак, що не покладається на конкретні сигнатури.
Наприклад, ви можете перевірити, чи певні користувачі здійснюють підозрілі виклики API, які вони не повинні були б здійснювати, використовуючи регіони, які зазвичай не використовуються, входячи з підозрілих IP-адрес джерела або з підозрілими user agents тощо...
Після того, як ви виявили будь-які підозрілі виклики API, IP-адреси джерела чи user agents, ви можете швидко визначити, якими ключами доступу могли зловживати протягом цього часового проміжку, та опиратися на ці ключові слова в оригінальних журналах JSON для створення хронології діяльності зловмисника.

> Попередження: у клітинках буде багато даних, і вони, найімовірніше, не відображатимуться належним чином у програмах на кшталт Excel. Будь ласка, використовуйте Numbers на Mac, Timeline Explorer на Windows тощо...

### Приклад `AbusedAPIs-Success`:
```
...
> Зауважте, що оскільки буде велика кількість тимчасових ідентифікаторів ключів доступу AWS STS, вони відфільтровуються за замовчуванням. Додайте параметр `-s, --include-sts-keys`, якщо ви хочете їх включити.
...
> Зауважте, що клієнтський інструмент `aws` включає інформацію про ОС у user agent, тому можливо виявити, чи здійснювалися виклики API з ОС зловмисників на кшталт `kali`.
...
### Приклад команди `aws-ct-metrics`

* Збереження результатів у файл CSV: `./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
