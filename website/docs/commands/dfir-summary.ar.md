...
ينشئ هذا الأمر ملخصًا للمعلومات التالية بناءً على ARNs الخاصة بالمستخدمين:
  - العدد الإجمالي للأحداث (استدعاءات API) (`NumOfEvents`)
  - الطابع الزمني لأول استدعاء API موجود في السجلات (`FirstTimestamp`)
  - الطابع الزمني لآخر استدعاء API موجود في السجلات (`LastTimestamp`)
  - استدعاءات API التي يُساء استخدامها بشكل شائع والتي نجحت (`AbusedAPIs-Success`)
  - استدعاءات API التي يُساء استخدامها بشكل شائع والتي تمت محاولتها لكنها فشلت (`AbusedAPIs-Failed`)
  - استدعاءات API الأخرى التي ليست في قائمة استدعاءات API التي يُساء استخدامها بشكل شائع والتي نجحت (`OtherAPIs-Success`)
  - استدعاءات API الأخرى التي ليست في قائمة استدعاءات API التي يُساء استخدامها بشكل شائع والتي تمت محاولتها لكنها فشلت (`OtherAPIs-Failed`)
  - مناطق AWS التي تم فيها إجراء استدعاءات API (`AWS-Regions`)
  - عناوين IP المصدر لاستدعاء API (`SrcIPs`)
  - أنواع المستخدمين (`UserTypes`)
  - معرّفات مفاتيح الوصول للمستخدمين (`UserAccessKeyIDs`)
  - وكلاء المستخدم للمصدر الذي أجرى استدعاء API (`UserAgents`)

※ ملاحظة: استدعاءات API التي يُساء استخدامها بشكل شائع تأتي من ملف الإعداد المستضاف على [https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv](https://github.com/Yamato-Security/suzaku-rules/blob/main/config/abused_aws_api_calls.csv). سيتم تحديث هذا الملف بمرور الوقت وستتم مزامنته محليًا في كل مرة تقوم فيها بتشغيل أمر `update-rules`.

تهدف هذه النتائج إلى تزويد المحللين بمعلومات لاكتشاف الحسابات المخترقة أو الهجمات التي لا تعتمد على توقيعات محددة.
على سبيل المثال، يمكنك التحقق مما إذا كان مستخدمون معينون يجرون استدعاءات API مشبوهة لا ينبغي لهم إجراؤها، أو يستخدمون مناطق لا تُستخدم عادةً، أو يسجلون الدخول من عناوين IP مصدر مشبوهة أو بوكلاء مستخدم مشبوهين، إلخ...
بعد أن تعثر على أي استدعاءات API مشبوهة يتم إجراؤها، أو عناوين IP مصدر، أو وكلاء مستخدم، يمكنك تحديد مفاتيح الوصول التي ربما أُسيء استخدامها خلال تلك الفترة الزمنية بسرعة والاستناد إلى تلك الكلمات المفتاحية في سجلات JSON الأصلية لإنشاء جدول زمني لنشاط المهاجم.

> تحذير: ستكون هناك كمية كبيرة من البيانات في الخلايا ومن المرجح ألا تُعرض بشكل جيد في برامج مثل Excel. يُرجى استخدام Numbers على جهاز Mac، أو Timeline Explorer على نظام Windows، إلخ...

### مثال `AbusedAPIs-Success`:
```
...
> لاحظ أنه نظرًا لوجود كمية كبيرة من معرّفات مفاتيح الوصول المؤقتة لـ AWS STS، فإنها تُرشّح افتراضيًا. أضف خيار `-s, --include-sts-keys` إذا كنت تريد تضمينها.
...
> لاحظ أن أداة العميل `aws` ستُضمّن معلومات نظام التشغيل في وكيل المستخدم لذا من الممكن اكتشاف ما إذا كانت هناك استدعاءات API تم إجراؤها من أنظمة تشغيل المهاجمين مثل `kali`.
...
### مثال أمر `aws-ct-metrics`

* حفظ النتائج في ملف CSV: `./suzaku aws-ct-summary -d ../suzaku-sample-data -o sample-summary.csv`
