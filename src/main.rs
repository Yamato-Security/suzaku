...
impl App {
    ...
    fn exec(&mut self, app: &mut Command, stored_static: &mut StoredStatic) {
        ...
        if !CURRENT_EXE_PATH.join("config").exists() && !Path::new("./config").exists() {
            AlertMessage::alert(
                "Suzaku could not find the config directory.\nPlease make sure that it is in the same directory as the suzaku executable."
            )
            .ok();
            return;
        }
        ...
        match &stored_static.config.action.as_ref().unwrap() {
            ...
            Action::UpdateRules(_) => {
                ...
                let latest_version_data = if let Ok(data) = Update::get_latest_suzaku_version() {
                    data
                } else {
                    None
                };
        ...
        match stored_static.config.action {
            Some(Action::CsvTimeline(_)) | Some(Action::JsonTimeline(_)) => {
                println!();
                println!("Please report any issues with Suzaku rules to: https://github.com/Yamato-Security/suzaku-rules/issues");
