...
fn _output_html_computer_by_mitre_attck(html_output_stock: &mut Nested<String>) {
    ...
    if COMPUTER_MITRE_ATTCK_MAP.is_empty() {
        html_output_stock.push("- No computers were detected with MITRE ATT&CK Tactics.<br>Make sure you run Suzaku with a profile that includes %MitreTactics% in order to get this info.<br>");
    }
